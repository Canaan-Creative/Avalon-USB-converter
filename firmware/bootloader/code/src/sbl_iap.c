...
unsigned int write_flash(unsigned int dst, unsigned char *src, unsigned int no_of_bytes)
{
	...
	if (find_erase_prepare_sector(dst))
		ret = 1;
	if (write_data((dst), src, no_of_bytes))
		ret = 1;
...
unsigned int write_finish_upgrade_flag(void)
{
	...
	g_flash_buf[FLASH_BUF_SIZE - 1] = 0xAA;
	g_flash_buf[FLASH_BUF_SIZE - 2] = 0xBB;
	g_flash_buf[FLASH_BUF_SIZE - 3] = 0xCC;
	g_flash_buf[FLASH_BUF_SIZE - 4] = 0xDD;

	if ((get_write_flash_addr() + FLASH_BUF_SIZE) > (APP_END_ADDR - SECTOR_SIZE)) {
		if (find_erase_prepare_sector((APP_END_ADDR - FLASH_BUF_SIZE)))
			ret = 1;
	} else {
		if (find_erase_prepare_sector((APP_END_ADDR - SECTOR_SIZE)))
			ret = 1;
	}
