...
static unsigned int write_data(unsigned int flash_address, unsigned char *data_addr, unsigned int count)
{
...
static unsigned int erase_sector_usb(unsigned int start_sector, unsigned int end_sector)
{
	unsigned int param_table[5];
	unsigned int result_table[5];
...
static unsigned int prepare_sector_usb(unsigned int start_sector, unsigned int end_sector)
{
	unsigned int param_table[5];
	unsigned int result_table[5];
...
int write_updata_flag(void)
{
	...
	src_addr = APP_END_ADDR - FLASH_BUF_SIZE;
	memcpy((unsigned char *)(&g_flash_buf[0]), (unsigned char *)src_addr, FLASH_BUF_SIZE);

	g_flash_buf[FLASH_BUF_SIZE - 1] = 0x00;
	g_flash_buf[FLASH_BUF_SIZE - 2] = 0x00;
	g_flash_buf[FLASH_BUF_SIZE - 3] = 0x00;
	g_flash_buf[FLASH_BUF_SIZE - 4] = 0x00;

	if (find_erase_prepare_sector((APP_END_ADDR - FLASH_BUF_SIZE)))
		ret = 1;

	if (write_data((APP_END_ADDR - FLASH_BUF_SIZE), g_flash_buf, FLASH_BUF_SIZE))
		ret = 1;
