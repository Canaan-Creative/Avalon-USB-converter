...
static ErrorCode_t HID_Ep_Hdlr(USBD_HANDLE_T hUsb, void *data, uint32_t event)
{
	...
	case USB_EVT_OUT:
		...
		rxCnt = USBD_API->hw->ReadEP(hUsb, pHidCtrl->epout_adr, loopback_report);
		if (rxCnt) {
			/* loopback the report received. */
			USBD_API->hw->WriteEP(hUsb, pHidCtrl->epin_adr, loopback_report, rxCnt);
		}
		break;
	}
	return LPC_OK;
						 ...
						 uint32_t *mem_size)
{
	...
	hid_param.mem_base += HID_RX_BUF_SZ;
	hid_param.mem_size += HID_RX_BUF_SZ;
